...
# API Config
API_CONFIG = "http://localhost:8000/api/v1/telemetry/"

"""deprecated from csv output, moved to postgres
PH_ADDRESS = 0x63
EC_ADDRESS = 0x64
TEMP_ADDRESS = 0x66
...
class AtlasComm:
    ...
    # sends commands to sensor, waits, and then returns float or none
    def send_and_read(self, command='r', wait_time=1.0):

        try:
            self.target_sensor()

            self.file_write.write((command + "\0").encode('latin-1'))
            ...
            if data[0] == 1:
                return float(data[1:].decode('latin-1').rstrip('\x00'))
            return None

        except Exception as e:
            logging.warning(f"Sensor {hex(self.address)} read error: {e}")
            return None

"""deprecated data functions. Handled via postgres queries now
#data functions
...
# main
def main():
    logging.info(f"Bus {I2C_BUS}, we can scale more later.")
    ...
    sensors = {name: AtlasComm(address=addr, bus=I2C_BUS) for name, addr in sensor_suite.items()}

    session = requests.Session()

    """deprecated sensor suite map, redisigned
...
deprecated interval calculations and output, done through postgres queries in other scripts
    ...
    try:
        ...
        print("\nlogging stopped...")
    """
    # output loop to fastapi. Crop ID (bus) set to 1 for now.
    #future versions will be able to handle multiple bus IDs.
    try:
        while True:
            ...
            # this loop reads all sensors
            for name, sensor in sensors.items():
                payload[name] = sensor.send_and_read()

            logging.info(f"Sending payload: {payload}")

            #send payload to fastapi
            try:
                response = session.post(API_CONFIG, json=payload, timeout=5)
                ...
                logging.error(f"\n**unable to connect to fastapi {e}")
